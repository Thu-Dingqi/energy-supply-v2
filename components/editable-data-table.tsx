        ...
        <table className="w-full table-fixed">
          <thead>
            <tr className="bg-slate-100">
              <th className="w-[120px] px-4 py-2 text-left text-sm font-medium border-r">指标</th>
              <th className="w-[80px] px-4 py-2 text-left text-sm font-medium border-r">单位</th>
              {years.map((year) => (
                <th key={year} className="w-[80px] px-4 py-2 text-center text-sm font-medium border-r">
                  {year}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {editableData.map((row, rowIndex) => (
              <tr key={rowIndex} className="border-t hover:bg-slate-50">
                <td className="w-[120px] px-4 py-2 text-sm border-r font-medium truncate">{row.indicator}</td>
                <td className="w-[80px] px-4 py-2 text-sm border-r text-muted-foreground truncate">{row.unit}</td>
                {years.map((year) => (
                  <td
                    key={year}
                    className="w-[80px] px-4 py-2 text-sm text-center border-r"
                  ...
                  >
                    {editingCell?.rowIndex === rowIndex && editingCell?.year === year ? (
                      ...
                      <div className="cursor-pointer hover:bg-slate-100 p-1 rounded truncate">{row.values[year]}</div>
                    )}
                  </td>
                ))}
              </tr>
